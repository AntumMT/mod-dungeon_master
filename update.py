...
ref = "master"
mob_name = "dungeon_master"
mob_title = "Dungeon Master"
...
def updateNamespace(target):
	...
	for idx in reversed(range(len(lines))):
		li = lines[idx]
		if li.startswith("mobs:alias_mob(\"mobs:{}\"".format(mob_name)) or li == "-- spawn egg" or li == "-- old mobs mod compatibility":
			lines.pop(idx)
		elif li.startswith("mobs:register_egg("):
			lines.pop(idx)
			if len(lines) > idx:
				if lines[idx].lstrip().startswith("\"fire_basic_flame.png\""):
...
def updateConf():
	...
	for idx in range(len(lines)):
		...
		if li.startswith("name ="):
			...
			if "fire" not in li.replace(" ", "").split("=")[1].split(","):
				lines[idx] = lines[idx] + ", fire"
...
def updateTextures():
	...
	for idx in range(2, 50):
		...
		if download(getRefUrl(file_path), file_path, False) != 0:
			break
	download(getRefUrl("textures/mobs_fireball.png"), "textures/mobs_fireball.png")
	for n in ("nether", "netherdeep"):
		file_path = "textures/{}_{}.png".format(file_basename, n)
		download(getRefUrl(file_path), file_path)
...
if __name__ == "__main__":
	...
	download(getRefUrl("sounds/mobs_fireball.ogg"), "sounds/mobs_fireball.ogg")
